...
public class ParameterTargetService {
    ...
    public record Target(String parameter, String categoryCode, String phase, BigDecimal warnMin, BigDecimal warnMax,
                         BigDecimal critMin, BigDecimal critMax) {

        /** Higher = more specific; -1 = does not apply to this content. */
        int score(String category, String contentPhase) {
            ...
            return (categoryMatches ? 2 : 0) + (phaseMatches ? 1 : 0);
        }
    }
    ...
    public ParameterTargetService(JdbcTemplate jdbc, AuditService audit) {
        this.jdbc = jdbc;
        this.audit = audit;
    }
    ...
    public TargetView create(TargetRequest r) {
        ...
        requireUnique(parameterId, categoryId, blankToNull(r.phase()), null);
        try {
            jdbc.update("insert into parameter_target(id, parameter_id, category_id, phase, warn_min, warn_max, crit_min, "
                    + "crit_max, note) values (?, ?, ?, ?, ?, ?, ?, ?, ?)", id, parameterId,
                categoryId, blankToNull(r.phase()), r.warnMin(), r.warnMax(), r.critMin(), r.critMax(),
                blankToNull(r.note()));
    ...
    public TargetView update(UUID id, TargetRequest r) {
        ...
        UUID parameterId = parameterId(r.parameter());
        UUID categoryId = categoryId(r.categoryCode());
        requireUnique(parameterId, categoryId, blankToNull(r.phase()), id);
        try {
            jdbc.update("update parameter_target set parameter_id = ?, category_id = ?, phase = ?, warn_min = ?, warn_max = ?, "
    ...
    public List<Target> all() {
        return jdbc.query("select p.code param, c.code cat, t.phase, t.warn_min, t.warn_max, t.crit_min, t.crit_max "
                + "from parameter_target t join parameter p on p.id = t.parameter_id "
                + "left join internal_category c on c.id = t.category_id",
            (rs, n) -> new Target(rs.getString("param"), rs.getString("cat"), rs.getString("phase"),
    ...
    public static Target resolve(List<Target> targets, String parameter, String category, String phase) {
        return targets.stream()
            .filter(t -> t.parameter().equals(parameter) && t.score(category, phase) >= 0)
            .max(Comparator.comparingInt(t -> t.score(category, phase)))
            .orElse(null);
    }
    ...
    private static final String VIEW_SQL = "select t.id, p.code param, p.name param_name, p.reference_unit unit, c.code cat_code, "
        + "c.name cat_name, t.phase, t.warn_min, t.warn_max, t.crit_min, t.crit_max, t.note from parameter_target t "
    ...
    private TargetView view(java.sql.ResultSet rs) throws java.sql.SQLException {
        return new TargetView(rs.getObject("id", UUID.class), rs.getString("param"), rs.getString("param_name"),
            rs.getString("unit"), rs.getString("cat_code"), rs.getString("cat_name"), rs.getString("phase"),
            rs.getBigDecimal("warn_min"), rs.getBigDecimal("warn_max"), rs.getBigDecimal("crit_min"),
    ...
    private void requireUnique(UUID parameterId, UUID categoryId, String phase, UUID excludeId) {
        Integer same = jdbc.queryForObject("select count(*) from parameter_target where parameter_id = ? "
                + "and coalesce(category_id, '00000000-0000-0000-0000-000000000000') = coalesce(?, '00000000-0000-0000-0000-000000000000') "
                + "and coalesce(phase, '') = coalesce(?, '') and (?::uuid is null or id <> ?::uuid)",
            Integer.class, parameterId, categoryId, phase, excludeId, excludeId);
        if (same != null && same > 0) {
            throw new ConflictException("DUPLICATE_TARGET", "Ya existe un objetivo para ese parámetro, categoría y fase.");
        }
    }
